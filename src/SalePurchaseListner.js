...
const Address = "0xb34c6cF7ECaB54Cc7Ee32b28171624BDB4b59B1A";
...
dexContract.events
  .TokenMinted((error, event) => {
    try {
      createMintBurnListing(client, event);
    } catch (e) {
      console.error(e);
    }
  })
...
async function createNFTListing(client, event) {
  client.connect();
  const eventDetails = {
...
async function createMintBurnListing(client, event) {
  client.connect();
  const eventDetails = {
    txHash: event.transactionHash,
    blockNumber: event.blockNumber.toString(),
